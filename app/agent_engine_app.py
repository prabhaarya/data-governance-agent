...
class AgentEngineApp(A2aAgent):
    ...
    def create(
        ...
        # --- PATCH START: Intercept Protobuf serialization to handle Pydantic models ---
        _orig_MessageToJson = json_format.MessageToJson

        def _patched_MessageToJson(message, *args, **kwargs):
            # If the object lacks a Protobuf descriptor, it's our Pydantic model
            if not hasattr(message, "DESCRIPTOR"):
                if hasattr(message, "model_dump_json"):
                    return message.model_dump_json(exclude_none=True)
                return message.json(exclude_none=True)
            
            # Otherwise, fall back to standard Protobuf serialization
            return _orig_MessageToJson(message, *args, **kwargs)

        # Apply patch globally for the deployment script
