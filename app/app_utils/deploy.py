    ...
    if kv_string:
        for pair in kv_string.split(","):
            if "=" in pair:
                ...
                # 💡 THE FIX: If the value is empty, give it a placeholder string.
                # This guarantees the SDK serializes a 'value' field to the API.
                if not cleaned_value:
                    logging.warning(f"Empty value detected for key '{cleaned_key}'. Skiiping it.")
                else:
...
@click.option(
    "--description",
    default="ReAct agent with A2A protocol",
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Enable agent identity for per-agent IAM access control",
)
@click.option(
    "--gemini-enterprise-app-id",
    default=None,
    help="Full resource name of the Gemini Enterprise App to register this agent to",
)
def deploy_agent_engine_app(
    ...
    matching_agents = [
        ...
        if agent.api_resource.description == description
    ]
    ...
    if gemini_enterprise_app_id:
        try:
            from app.app_utils.register_gemini_enterprise import execute_registration
        except ImportError:
            from register_gemini_enterprise import execute_registration

        resource_name_parts = remote_agent.api_resource.name.split("/")
        agent_engine_id = resource_name_parts[-1]
        agent_card_url = f"https://{location}-aiplatform.googleapis.com/v1beta1/projects/{project}/locations/{location}/reasoningEngines/{agent_engine_id}/a2a/v1/card"

        click.echo("\n📦 Triggering Gemini Enterprise A2A Registration...")
