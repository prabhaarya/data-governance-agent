...
deploy:
	...
	uv run -m app.app_utils.deploy \
		...
		$(if $(GEMINI_APP_ID),--gemini-enterprise-app-id="$(GEMINI_APP_ID)") \
...
setup-infra:
	...
	(cd deployment/terraform/ && terraform init && terraform apply --var-file vars/env.tfvars --var dev_project_id=$$PROJECT_ID --auto-approve)
